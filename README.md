...
`make desktop-package [RID=…] [VERSION=…]` packages the build with [Velopack](https://velopack.io) in `artifacts/packages/<rid>/`: an installer (`-Setup.exe` on Windows, `-Setup.pkg` on macOS, `.AppImage` on Linux), a portable `.zip`, and the update feed (`releases.<rid>.json` plus full and delta `.nupkg` packages).

Auto-update: an installed desktop app checks this repository's GitHub Releases shortly after startup and every six hours, downloads a newer version in the background, then shows a "Restart now" banner; the update is also applied when the app closes. Settings › API service shows the installed version and a "Check now" button. Development runs (`make desktop`) do not update. Set `ALA_UPDATE_SOURCE` to a folder or URL containing a Velopack feed to test updates locally.
